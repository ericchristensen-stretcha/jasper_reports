...
class Report:
    ...
    def execute(self):
        ...
        for subreportInfo in self.report.subreports():
            ...
            if subreport.language() == 'xpath':
                ...
                if subreport.isHeader():
                    _logger.info('isheader')
                    generator = CsvBrowseDataGenerator(subreport, 'res.users', self.pool, self.cr, self.uid, [self.uid], self.context)
                elif subreportInfo['model'] and subreportInfo['model'] != self.model:
                    if 'model' in subreportInfo:
                        _logger.info('subreport: %s, with model: %s', subreport, subreportInfo['model'])
                    if 'searchDomain' in subreportInfo:
                        _logger.info('searchDomain: %s', subreportInfo['searchDomain'])
                    if subreportInfo.get('searchDomain',False):
                        try:
                            domain_clause = eval( ast.literal_eval(subreportInfo['searchDomain']))
                            model_ids = self.pool.get(subreportInfo['model']).search(self.cr,self.uid,domain_clause,context=self.context)
                        except Exception as e:
                            _logger.info('error message domain clause: %s', str(e))
                            raise osv.except_osv(_('Error'), _('Domain clause (OPENERP_PREFILTER) could not be evaluated!')+' -|%s|-' % subreportInfo['searchDomain'])
                            
                    else:
                        model_ids = self.pool.get(subreportInfo['model']).search(self.cr,self.uid,[('id','!=',0)],context=self.context)
                    _logger.info('selected ids: %s', model_ids)
                    generator = CsvBrowseDataGenerator(subreport, subreportInfo['model'], self.pool, self.cr, self.uid, model_ids, self.context)
                    ...
                    _logger.info('niks subreport: %s, model: %s, ids: %s', subreport, self.model, self.ids)
        ...
        _logger.info('dataFile: %s',dataFile)
        _logger.info('subreportDataFiles: %s',subreportDataFiles)
        for subreportdf in subreportDataFiles:
            fh = open(subreportdf['dataFile'])
            _logger.info('Content: %s', fh.read())
            fh.close()
    ...
    def executeReport(self, dataFile, outputFile, subreportDataFiles):
        ...
        _logger.debug('connectionParameters: %s', connectionParameters)
