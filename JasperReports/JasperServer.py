...
class JasperServer:
    ...
    def start(self):
        ...
        command = ['java', '-Djava.awt.headless=true', '-Xms512M', '-Xmx1024M', 'com.nantic.jasperreports.JasperServer', unicode(self.port)]
        output_fh = open('/tmp/java_process_output.txt','w')
        process = subprocess.Popen(command, env=env, cwd=cwd, stdout=output_fh)
        if self.pidfile:
