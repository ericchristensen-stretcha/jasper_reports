...
class JasperReport:
    ...
    def extractProperties(self):
        ...
        _logger.info('subreporttags: %s', subreportTags)
        for tag in subreportTags:
            # this makes shure the tag only contains exactly what we want it to contain
            # an error was noticed retrieving the model from a subsequent tag and it still
            # returned a wrong value
            tag = etree.fromstring(etree.tostring(tag))
            _logger.info('tag text: %s', etree.tostring(tag, encoding='utf8', method='xml'))
            dataSourceExpression = tag.findtext('{%s}dataSourceExpression' % ns, '')
            connectionExpression = tag.findtext('{%s}connectionExpression' % ns, '')
            if connectionExpression:
                dataSourceExpression = connectionExpression.strip()
                m = dataSourceExpressionRegExp.match( dataSourceExpression )
                dataSourceExpression = m.group(1)
            elif dataSourceExpression:
                _logger.info('dataexpression: %s', dataSourceExpression)
                # dataSourceExpression == 'REPORT_DATA_SOURCE'
            ...
            _logger.info('beyond expressions!')
            # Model

            _logger.info('model tag text: %s', etree.tostring(tag, encoding='utf8', method='xml'))
            model = ''
            ...
            if modelTags and 'value' in modelTags[0].keys():
                ...
                _logger.info('Model is set to: %s', model)
            _logger.info('Final Model: %s', model)
            ...
            searchDomain = []
            # searchDomainTags = tag.xpath( '//jr:subreportParameter[@name="OPENERP_PREFILTER"]', namespaces=nss )
            # if searchDomainTags:
                # _logger.info('searchDomainTags text: %s', etree.tostring(searchDomainTags[0], encoding='utf8', method='xml'))
                # searchDomain = searchDomainTags[0].find('jr:subreportParameterExpression', namespaces=nss).text
            # else:
                # _logger.info('No searchDomain tags found')
            searchDomainTags = tag.xpath( '//jr:reportElement/jr:property[@name="OPENERP_PATH_PREFIX"]', namespaces=nss )
            if searchDomainTags and 'value' in searchDomainTags[0].keys():
                searchDomain = searchDomainTags[0].get('value')
            _logger.info('searchDomain: %s', searchDomain)
        ...
        _logger.info("subreport info: %s", self._subreports)
        # Dataset
        # Here we expect the following structure in the .jrxml file:
        #<datasetRun>
        #  <dataSourceExpression><![CDATA[$P{REPORT_DATA_SOURCE}]]></dataSourceExpression>
